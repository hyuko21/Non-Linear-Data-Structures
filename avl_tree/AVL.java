public class AVL extends BST implements AVLInterface {
	public AVL() {
		super();
	}
	public AVLNode root() {
  ...
  private void rotationLeft(AVLNode v) {
  	...
  	if (rightChild != null) {
  		...
  		leftChild = (AVLNode) rightChild.leftChild();
  ...
  private void rotationRight(AVLNode v) {
  	// atualizar fb, quando rotação
  	rotbal(v, 1);
  	...
  	if (leftChild != null) {
	  	...
	  	rightChild = (AVLNode) leftChild.rightChild();
  ...
  private void doubleRotationLeft(AVLNode v) {
  	// rebalancear à direita
  	rotationRight((AVLNode) v.rightChild());
  	
  	// rebalancear à esquerda
  	rotationLeft((AVLNode) v);
  }
  private void doubleRotationRight(AVLNode v) {
  	// rebalancear à esquerda
  	rotationLeft((AVLNode) v.leftChild());
  	
  	// rebalancear à direita
  	rotationRight((AVLNode) v);
  }
  @Override
  ...
  protected Object removeNode(int k) {
    ...
    if (k == nk) {
      ...
      // Tem apenas um filho (esquerda ou direita)
      ...
      // Tem dois filhos
        ...
        AVLNode w = (AVLNode) getLLD(v.rightChild()); // get last left descendant
        removeNode(key(w));
        o = v.element();
        swap(v, w);
      }
